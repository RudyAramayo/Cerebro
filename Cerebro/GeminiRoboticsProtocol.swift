...
enum GeminiRoboticsProtocol {
    ...
    private static let armControlToolDeclaration: [String: Any] = [
        "name": "arm_control",
        "description": "For explicit addressed arm requests: relax gently returns to hanging then deactivates; prepare brings both arms in front and calibrates both empty grippers; grab/hold prepares then uses the cameras to attempt a low-intensity close only with the requested object between the jaws. Fixed local routines are already authorized by the user's command. No raw joint values, force values, extra approval or controller grant. General reaching is unavailable. status is read-only. Never claim a grip attempt is a verified grasp.",
        "behavior": "BLOCKING",
        "parameters": [
            "type": "OBJECT",
            "properties": [
