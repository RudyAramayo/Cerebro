...
enum ROBControlAuthorizationPolicy {
  static func allowsInbound(_ type: DataMessageType, for role: ROBControlPeerRole) -> Bool {
    switch (role, type) {
    case (.operatorController, .sendData), (.lidarPublisher, .lidarTelemetry):
      return true
    default:
      return false
    }
...
struct ROBLidarTelemetryMessage: Codable, Equatable {
  ...
  func validationError(
    ...
    guard schemaVersion == Self.currentSchemaVersion else { return "unsupported schema version" }
    guard deviceID == authenticatedDeviceID else { return "device ID does not match authenticated peer" }
    guard sequence > 0, sequence > lastAcceptedSequence else { return "sequence is missing or not increasing" }
    ...
    case .scan:
      guard let scanPayload,
        ...
        mapData == nil, mapWidth == nil, mapHeight == nil
      else { return "scan payload is missing, oversized, or mixed with map data" }
    case .map:
      guard scanPayload == nil,
        let mapData, !mapData.isEmpty, mapData.count <= Self.maximumMapDataBytes,
        let mapWidth, let mapHeight,
        (1...Self.maximumMapDimension).contains(mapWidth),
        (1...Self.maximumMapDimension).contains(mapHeight)
      else { return "map payload or dimensions are invalid" }
      let (cellCount, overflow) = mapWidth.multipliedReportingOverflow(by: mapHeight)
      guard !overflow, mapData.count >= cellCount else { return "map data is smaller than its dimensions" }
    }
    return nil
  }
}
