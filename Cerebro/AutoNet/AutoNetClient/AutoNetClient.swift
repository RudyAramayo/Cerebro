...
@objcMembers public class AutoNetClient: NSObject, AutoNetClientConnectionDelegate {
    ...
    public init(host: String, port: UInt16) {
        ...
        let frameOptions = NWProtocolFramer.Options(definition: CTDataTransferProtocol.definition)
        ...
        if let host = self.host, let port = self.port {
            ...
            connection = CTNWClientConnection(nwConnection: nwConnection)
        }
    }
    ...
    public func startBrowsing() {
        ...
        browser.browseResultsChangedHandler = { results, changes in
            ...
            if let browseResult = result,
                ...
                self.connection = CTNWClientConnection(nwConnection: nwConnection)
                self.start()
