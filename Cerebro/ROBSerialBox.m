...
static NSString * const kROBPersonTrackingCenteringSource = @"Torso face tracking";
static NSTimeInterval const kROBPersonTrackingObservationSettleSeconds = 0.1;
...
static NSString * const kROBPersonTrackingPostureSource =
    @"Person tracking posture sequence";
...
@property (readwrite, assign, getter=isPersonTrackingPostureSequenceActive)
    BOOL personTrackingPostureSequenceActive;
@property (readwrite, copy) NSArray<NSString *> *personTrackingPostureNames;
@property (readwrite, copy) NSArray<ROBServoCameraPosition *> *personTrackingPosturePositions;
@property (readwrite, assign) NSUInteger personTrackingPostureIndex;
@property (readwrite, assign) NSUInteger personTrackingPostureGeneration;
@property (readwrite, assign) BOOL personTrackingPostureAdvanceScheduled;
@property (readwrite, assign) NSTimeInterval personTrackingPostureDeadline;
...
@property (readwrite, assign) NSTimeInterval lastNeckTraceWriteUptime;
@property (readwrite, retain) ROBServoSequencePhase *safeNeckStartupPhaseOne;
@property (readwrite, retain) ROBServoSequencePhase *safeNeckStartupPhaseTwo;
@property (readwrite, retain) ROBServoSequencePhase *safeNeckStartupPhaseThree;
...
- (void)traceNeckWriteForSource:(NSString *)source
                 firstChannel:(NSInteger)channel
                      targets:(NSArray<NSNumber *> *)targets;
...
- (BOOL)applyNeckSafetyConfiguration:(ROBNeckSafetyConfig)configuration
{
    ...
    self.lastNeckTraceWriteUptime = 0;
...
- (void)invalidateNeckCommandStateWithStatus:(NSString *)status
{
    ...
    self.safeNeckStartupPhaseOne = nil;
    self.safeNeckStartupPhaseTwo = nil;
...
- (void)attemptMaestroReconnect
{
    ...
    if (self.maestroConnectionValid) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // Hardware readiness owns hardware startup. Do not make servo
                        ...
                        accelerationLimit:(NSInteger)accelerationLimit
{
    ...
    @synchronized (self) {
        ...
        if (self.commandedLowerNeckTargetReadyAt > now) {
            self.commandedLowerNeckTargetReadyAt = fmax(
                self.commandedLowerNeckTargetReadyAt,
                now + kROBNeckClearanceSettleSeconds + worstLowerDuration
            );
        }
...
- (void)traceNeckWriteForSource:(NSString *)source
                 firstChannel:(NSInteger)channel
                      targets:(NSArray<NSNumber *> *)targets
{
    // Opt in with the launch argument -ROBNeckCommandTrace YES. Log successful
    // wire writes, not render requests or assumed physical motion.
    NSTimeInterval now = NSProcessInfo.processInfo.systemUptime;
    if ([NSUserDefaults.standardUserDefaults boolForKey:@"ROBNeckCommandTrace"]) {
        NSLog(@"Neck TX uptime=%.3f dt=%.3f source=%@ channel=%ld targets=%@ speed=%ld acceleration=%ld",
              now, self.lastNeckTraceWriteUptime > 0 ? now - self.lastNeckTraceWriteUptime : 0,
              source, (long)channel, [targets componentsJoinedByString:@","],
              (long)(self.maestroServoSmoothingEnabled ? self.maestroServoSpeedLimit : 0),
              (long)(self.maestroServoSmoothingEnabled ? self.maestroServoAccelerationLimit : 0));
    }
    self.lastNeckTraceWriteUptime = now;
}
...
- (ROBNeckCommandDisposition)startSafeNeckStartup
{
    ...
    // in-flight safety sequence must never change underneath the mechanism.
    ROBServoControlStore *store = [ROBServoControlStore shared];
    ROBServoSequencePhase *phaseOne = [store startupPhaseAtIndex:0];
    ROBServoSequencePhase *phaseTwo = [store startupPhaseAtIndex:1];
    ROBServoSequencePhase *phaseThree = [store startupPhaseAtIndex:2];
    BOOL phasesAreStructurallySafe = phaseOne != nil
        && phaseTwo != nil
        && phaseThree != nil
        && phaseOne.phaseIndex == 1
        && phaseTwo.phaseIndex == 2
        && phaseThree.phaseIndex == 3
        && phaseOne.panTarget == ROBNeckSafetyTargetOff
        && phaseTwo.panTarget != ROBNeckSafetyTargetOff
        && phaseTwo.lowerTarget == phaseOne.lowerTarget
        && phaseTwo.upperTarget == phaseOne.upperTarget
        && phaseThree.panTarget == phaseTwo.panTarget;
    ...
    // rebased after the final pose settles so it cannot alter any phase.
    startupConfiguration.cameraLevelingEnabled = false;
    ROBNeckSafetyResult phaseOnePose = {0};
    ROBNeckSafetyResult phaseTwoPose = {0};
    ROBNeckSafetyResult phaseThreePose = {0};
    BOOL targetsAreValid = phasesAreStructurallySafe
        && ROBNeckSafetyLowerTargetHasFullPanClearance(
            &startupConfiguration,
            (int)phaseOne.lowerTarget
        )
        && ROBNeckSafetyApply(
            &startupConfiguration,
            (int)phaseOne.panTarget,
            (int)phaseOne.lowerTarget,
            (int)phaseOne.upperTarget,
            &phaseOnePose
        )
        && ROBNeckSafetyApply(
            &startupConfiguration,
            (int)phaseTwo.panTarget,
            (int)phaseTwo.lowerTarget,
            (int)phaseTwo.upperTarget,
            &phaseTwoPose
        )
        && ROBNeckSafetyApply(
            &startupConfiguration,
            (int)phaseThree.panTarget,
            (int)phaseThree.lowerTarget,
            (int)phaseThree.upperTarget,
            &phaseThreePose
        )
        && phaseOnePose.panTarget == phaseOne.panTarget
        && phaseOnePose.lowerTarget == phaseOne.lowerTarget
        && phaseOnePose.upperTarget == phaseOne.upperTarget
        && phaseTwoPose.panTarget == phaseTwo.panTarget
        && phaseTwoPose.lowerTarget == phaseTwo.lowerTarget
        && phaseTwoPose.upperTarget == phaseTwo.upperTarget
        && phaseThreePose.panTarget == phaseThree.panTarget
        && phaseThreePose.lowerTarget == phaseThree.lowerTarget
        && phaseThreePose.upperTarget == phaseThree.upperTarget
        && !phaseOnePose.panClamped
        && !phaseOnePose.lowerClamped
        && !phaseOnePose.upperClamped
        && !phaseTwoPose.panClamped
        && !phaseTwoPose.lowerClamped
        && !phaseTwoPose.upperClamped
        && !phaseThreePose.panClamped
        && !phaseThreePose.lowerClamped
        && !phaseThreePose.upperClamped;
    ...
    @synchronized (self) {
        ...
        self.safeNeckStartupReadyAt = 0;
        self.safeNeckStartupPhaseOne = phaseOne;
        self.safeNeckStartupPhaseTwo = phaseTwo;
        self.safeNeckStartupPhaseThree = phaseThree;
    ...
    ROBNeckCommandDisposition disposition = [self
        applySafeNeckPanTarget:(int)phaseOne.panTarget
    ...
    @synchronized (self) {
        ...
        readyAt += phaseOne.holdSeconds;
        self.safeNeckStartupReadyAt = readyAt;
        self.neckCommandSource = @"Torso safe startup";
        self.neckCommandSafetyStatus = [NSString stringWithFormat:
            @"SAFE STARTUP 1/3: PAN OFF; LOWER %ld • UPPER %ld",
            (long)phaseOne.lowerTarget,
            (long)phaseOne.upperTarget];
    }
    [[NSNotificationCenter defaultCenter]
...
- (void)advanceSafeNeckStartupForGeneration:(NSUInteger)generation
{
    ...
    NSTimeInterval readyAt = 0;
    ROBServoSequencePhase *phaseOne = nil;
    ROBServoSequencePhase *phaseTwo = nil;
    ROBServoSequencePhase *phaseThree = nil;
    @synchronized (self) {
        if (!self.safeNeckStartupInProgress
            || self.safeNeckStartupGeneration != generation) {
            return;
        ...
        phaseOne = self.safeNeckStartupPhaseOne;
        phaseTwo = self.safeNeckStartupPhaseTwo;
        phaseThree = self.safeNeckStartupPhaseThree;
    }
    if (phaseOne == nil || phaseTwo == nil || phaseThree == nil) {
    ...
    if (phase == ROBSafeNeckStartupPhaseLowering) {
        ROBNeckSafetyConfig configuration = [self neckSafetyConfiguration];
        ROBNeckSafetyPanBounds finalBounds = {0};
        BOOL finalCommandIsExact = self.neckPanCommandKnown
            && self.lowerNeckTiltCommandKnown
            && self.upperNeckTiltCommandKnown
            && self.commandedNeckPanTarget == phaseThree.panTarget
            && self.commandedLowerNeckTiltTarget == phaseThree.lowerTarget
            && self.commandedUpperNeckTiltTarget == phaseThree.upperTarget
            && ROBNeckSafetyAllowedPanBounds(
                &configuration,
                (int)phaseThree.lowerTarget,
                &finalBounds
        ...
        @synchronized (self) {
            ...
            if (!finalCommandIsExact) {
                ...
                self.torsoNeckAuthorityRequiresOperatorAction = YES;
                self.neckCommandSafetyStatus =
                    @"SAFE STARTUP INTERRUPTED: phase-3 command could not be verified.";
                return;
            }
            self.panEnvelopeLowerTarget = (int)phaseThree.lowerTarget;
            self.panEnvelopeLowerTargetIsKnown = YES;
            self.pendingPanEnvelopeLowerTarget = ROBNeckSafetyTargetOff;
            self.pendingPanEnvelopeReadyAt = 0;
            self.currentNeckPanMinimumDegrees = finalBounds.minimumDegrees;
            self.currentNeckPanMaximumDegrees = finalBounds.maximumDegrees;
            self.neckLevelingReferenceIsValid = NO;
            self.lastDesiredUpperNeckTarget = (int)phaseThree.upperTarget;
            self.lastDesiredUpperNeckTargetIsKnown = YES;
            self.upperNeckCommandCompensated = NO;
            self.safeNeckStartupInProgress = NO;
            self.safeNeckStartupPhase = ROBSafeNeckStartupPhaseInactive;
            ...
            self.neckCommandSource = @"Torso safe startup";
            self.neckCommandSafetyStatus = [NSString stringWithFormat:
                @"SAFE STARTUP COMPLETE: P %ld • L %ld • U %ld (COMMANDS SETTLED BY WORST-CASE TIMING)",
                (long)phaseThree.panTarget,
                (long)phaseThree.lowerTarget,
                (long)phaseThree.upperTarget];
        }
        [[NSNotificationCenter defaultCenter]
            postNotificationName:ROBSafeNeckStartupCommandDidChangeNotification
                          object:self];
    ...
    if (phase == ROBSafeNeckStartupPhaseLifting) {
        ...
        // Establish only the forward pan target while lower remains at the
        // configured full-clearance phase-1 pose. Asking for the final lower target in the
        // same request made the generic recenter latch responsible for phase
        // progression and could leave startup held indefinitely.
        ROBNeckCommandDisposition disposition = [self
        ...
        @synchronized (self) {
            ...
            BOOL centeringCommandIsExact =
                disposition != ROBNeckCommandDispositionRejected
                && self.commandedNeckPanTarget
                    == phaseTwo.panTarget
                && self.commandedLowerNeckTiltTarget
                    == phaseTwo.lowerTarget
                && self.commandedUpperNeckTiltTarget
                    == phaseTwo.upperTarget;
            if (!centeringCommandIsExact) {
                self.safeNeckStartupGeneration += 1;
                self.safeNeckStartupInProgress = NO;
                self.safeNeckStartupPhase = ROBSafeNeckStartupPhaseInactive;
                self.safeNeckStartupReadyAt = 0;
                ...
                self.torsoNeckAuthorityRequiresOperatorAction = YES;
                self.neckCommandSafetyStatus =
                    @"SAFE STARTUP INTERRUPTED: forward clearance command could not be verified; manual controls are available.";
                return;
            }
            nextReadyAt = fmax(
                self.commandedNeckPanTargetReadyAt,
            ...
            nextReadyAt += phaseTwo.holdSeconds;
            self.safeNeckStartupReadyAt = nextReadyAt;
            self.neckCommandSource = @"Torso safe startup";
            self.neckCommandSafetyStatus = [NSString stringWithFormat:
                @"SAFE STARTUP 2/3: LOWER HELD AT %ld; CENTERING PAN TO %ld",
                (long)phaseTwo.lowerTarget,
                (long)phaseTwo.panTarget];
        }
        [[NSNotificationCenter defaultCenter]
            postNotificationName:ROBSafeNeckStartupCommandDidChangeNotification
                          object:self];
    ...
    if (phase != ROBSafeNeckStartupPhaseCentering) {
        [self cancelSafeNeckStartup];
        return;
    }

    // Pan has now settled at the reviewed phase-2 target. Submit the configured
    // final camera pose through the same gateway as a separate operation.
    ROBNeckCommandDisposition disposition = [self
        applySafeNeckPanTarget:(int)phaseThree.panTarget
    ...
    @synchronized (self) {
        ...
        BOOL loweringCommandIsExact =
            disposition == ROBNeckCommandDispositionAppliedCommand
            && self.commandedNeckPanTarget
                == phaseThree.panTarget
            && self.commandedLowerNeckTiltTarget
                == phaseThree.lowerTarget
            && self.commandedUpperNeckTiltTarget
                == phaseThree.upperTarget;
        if (!loweringCommandIsExact) {
            self.safeNeckStartupGeneration += 1;
            self.safeNeckStartupInProgress = NO;
            self.safeNeckStartupPhase = ROBSafeNeckStartupPhaseInactive;
            self.safeNeckStartupReadyAt = 0;
            ...
            self.torsoNeckAuthorityRequiresOperatorAction = YES;
            self.neckCommandSafetyStatus =
                @"SAFE STARTUP INTERRUPTED: phase-3 command was held or rejected; manual controls are available.";
            return;
        }
        ...
        nextReadyAt = fmax(nextReadyAt, self.commandedUpperNeckTargetReadyAt);
        nextReadyAt = fmax(nextReadyAt, now + kROBNeckClearanceSettleSeconds);
        nextReadyAt += phaseThree.holdSeconds;
        self.safeNeckStartupPhase = ROBSafeNeckStartupPhaseLowering;
        self.neckCommandSafetyStatus = [NSString stringWithFormat:
            @"SAFE STARTUP 3/3: LEAN FORWARD • P %ld • L %ld • U %ld",
            (long)phaseThree.panTarget,
            (long)phaseThree.lowerTarget,
            (long)phaseThree.upperTarget];
        self.safeNeckStartupReadyAt = nextReadyAt;
...
- (NSTimeInterval)neckCommandReadyAtUptime
{
    @synchronized (self) {
        NSTimeInterval readyAt = fmax(
            self.commandedNeckPanTargetReadyAt,
            self.commandedLowerNeckTargetReadyAt
        );
...
- (BOOL)personTrackingMayUpdateNeck
{
    @synchronized (self) {
        ...
        return self.maestroConnectionValid
            ...
            && self.commandedLowerNeckTiltTarget != ROBNeckSafetyTargetOff
            && self.commandedUpperNeckTiltTarget != ROBNeckSafetyTargetOff
            && !self.safeNeckStartupInProgress
            && !self.personTrackingUprightTransitionActive
            && !self.personTrackingPostureSequenceActive
            && !self.torsoNeckAuthorityRequiresOperatorAction
            && !([self.neckCommandSource isEqualToString:kROBFollowTrackingClearanceSource]
...
- (NSTimeInterval)personTrackingCorrectionReadyAtUptime
{
    @synchronized (self) {
        // Pan/upper deadlines include a full collision staging margin. Normal
        // centering needs the configured ramp plus one fresh-observation beat,
        // while lower motion and envelope changes retain their full safety wait.
        NSTimeInterval rampEnd = fmax(0, fmax(
            self.commandedNeckPanTargetReadyAt - kROBNeckPanRecenterSeconds,
            self.commandedUpperNeckTargetReadyAt - kROBNeckPanRecenterSeconds));
        NSTimeInterval readyAt = fmax(
            rampEnd + kROBPersonTrackingObservationSettleSeconds,
            fmax(self.commandedLowerNeckTargetReadyAt, self.pendingPanEnvelopeReadyAt));
        if (self.panRecenterSettleGate.active) {
            readyAt = fmax(readyAt, self.panRecenterSettleGate.readyAt);
        }
        return readyAt;
                                       ...
                                       desiredUpperTarget:(NSInteger)upperTarget
{
    ...
    @synchronized (self) {
        if (!self.personTrackingMayUpdateNeck
            || NSProcessInfo.processInfo.systemUptime
                < self.personTrackingCorrectionReadyAtUptime) {
            return ROBNeckCommandDispositionHeldForSafety;
        }
        return [self applySafeNeckPanTarget:(int)panTarget
                           lowerTiltTarget:(int)self.commandedLowerNeckTiltTarget
                         desiredUpperTarget:(int)upperTarget
                               includeLower:NO
              allowSupervisedLowerRecovery:NO
                                    source:kROBPersonTrackingCenteringSource];
    }
}
                                                  ...
                                                  upperTarget:(NSInteger)upperTarget
{
    if (![NSThread isMainThread]) {
        self.neckCommandSafetyStatus =
            @"Servo Control command rejected: operator action must arrive on the main thread.";
        return ROBNeckCommandDispositionRejected;
    }
    if (panTarget < ROBNeckSafetyTargetOff
        ...
        || upperTarget > ROBNeckSafetyMaximumMaestroTarget) {
        self.neckCommandSafetyStatus =
            @"Servo Control command rejected: one or more targets are outside the Maestro range.";
        return ROBNeckCommandDispositionRejected;
    }
    if (!self.maestroConnectionValid) {
        self.neckCommandSafetyStatus =
            @"Servo Control command waiting for the Maestro connection.";
        return ROBNeckCommandDispositionRejected;
    }
    if (self.safeNeckStartupInProgress) {
        [self cancelSafeNeckStartup];
...
{
    self.personTrackingPostureSequenceActive = NO;
    self.personTrackingPostureAdvanceScheduled = NO;
    self.personTrackingPostureNames = nil;
    self.personTrackingPosturePositions = nil;
...
- (void)schedulePersonTrackingPostureAdvance
{
    if (!self.personTrackingPostureSequenceActive
        || self.personTrackingPostureAdvanceScheduled) {
        return;
    }
    self.personTrackingPostureAdvanceScheduled = YES;
    NSUInteger generation = self.personTrackingPostureGeneration;
    NSTimeInterval now = NSProcessInfo.processInfo.systemUptime;
    NSTimeInterval delay = fmax(0.05, self.neckCommandReadyAtUptime - now + 0.05);
    __weak typeof(self) weakSelf = self;
    dispatch_after(
        dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)),
        dispatch_get_main_queue(),
        ^{
            typeof(self) strongSelf = weakSelf;
            if (strongSelf == nil
                || strongSelf.personTrackingPostureGeneration != generation) {
                return;
            }
            strongSelf.personTrackingPostureAdvanceScheduled = NO;
            [strongSelf advancePersonTrackingPostureSequence];
        }
    );
}
...
- (ROBNeckCommandDisposition)advancePersonTrackingPostureSequence
{
    ...
    NSTimeInterval now = NSProcessInfo.processInfo.systemUptime;
    if (now > self.personTrackingPostureDeadline) {
        [self cancelPersonTrackingPostureSequence];
        self.neckCommandSafetyStatus =
            @"Person tracking posture sequence timed out before all safe poses settled.";
        return ROBNeckCommandDispositionRejected;
    }

    ROBNeckSafetyConfig configuration = [self neckSafetyConfiguration];
    while (self.personTrackingPostureIndex
            < self.personTrackingPosturePositions.count) {
        ROBServoCameraPosition *position = self.personTrackingPosturePositions[
            self.personTrackingPostureIndex];
        BOOL exactAndSettled = self.neckPanCommandKnown
            && self.commandedNeckPanTarget == configuration.panCenterTarget
            && self.lowerNeckTiltCommandKnown
            && self.commandedLowerNeckTiltTarget == position.lowerTarget
            && self.upperNeckTiltCommandKnown
            && self.commandedUpperNeckTiltTarget == position.upperTarget
            && now >= self.neckCommandReadyAtUptime;
        if (!exactAndSettled) break;
        self.personTrackingPostureIndex += 1;
    }
    if (self.personTrackingPostureIndex
        >= self.personTrackingPosturePositions.count) {
        NSArray<NSString *> *completedNames = self.personTrackingPostureNames;
        [self cancelPersonTrackingPostureSequence];
        self.neckCommandSafetyStatus = [NSString stringWithFormat:
            @"PERSON TRACKING POSTURE COMPLETE: %@",
            [completedNames componentsJoinedByString:@" → "]];
        return ROBNeckCommandDispositionAppliedCommand;
    }
    ...
    ROBNeckCommandDisposition disposition = [self
        ...
        lowerTiltTarget:(int)position.lowerTarget
        desiredUpperTarget:(int)position.upperTarget
        includeLower:YES
        allowSupervisedLowerRecovery:NO
        source:kROBPersonTrackingPostureSource];
    if (disposition == ROBNeckCommandDispositionRejected) {
        NSString *rejection = self.neckCommandSafetyStatus;
        [self cancelPersonTrackingPostureSequence];
        self.neckCommandSafetyStatus = [NSString stringWithFormat:
            @"Person tracking posture %@ rejected: %@",
            position.name,
    ...
    self.neckCommandSafetyStatus = [NSString stringWithFormat:
        ...
        (unsigned long)self.personTrackingPosturePositions.count,
        position.name,
        configuration.panCenterTarget,
        (long)position.lowerTarget,
        (long)position.upperTarget];
    [self schedulePersonTrackingPostureAdvance];
    return disposition;
}
    ...
    (NSArray<NSString *> *)positionNames
{
    ...
    for (NSArray<NSString *> *allowed in allowedOrders) {
        if ([positionNames isEqualToArray:allowed]) {
            orderIsReviewed = YES;
            break;
        }
    }
    if (!orderIsReviewed) {
        self.neckCommandSafetyStatus =
            @"Person tracking posture order is not one of the reviewed distance/search sequences.";
        return ROBNeckCommandDispositionRejected;
    }
    if (self.personTrackingPostureSequenceActive) {
        self.neckCommandSafetyStatus = [self.personTrackingPostureNames
            isEqualToArray:positionNames]
            ? @"Person tracking posture sequence is already advancing."
            : @"Person tracking is finishing its current posture sequence first.";
        return ROBNeckCommandDispositionHeldForSafety;
    }
    ...
    if (self.safeNeckStartupInProgress
        ...
        || self.torsoNeckAuthorityRequiresOperatorAction) {
        self.neckCommandSafetyStatus =
            @"Person tracking posture sequence is waiting for current neck authority to end.";
        return ROBNeckCommandDispositionHeldForSafety;
    }
    ...
    ROBNeckSafetyConfig exactConfiguration = [self neckSafetyConfiguration];
    exactConfiguration.cameraLevelingEnabled = false;
    NSMutableArray<ROBServoCameraPosition *> *positions = [NSMutableArray array];
    for (NSString *name in positionNames) {
        ROBServoCameraPosition *position = [store cameraPositionNamed:name];
        ROBNeckSafetyResult exactPose = {0};
        BOOL positionIsExact = position != nil
            && ROBNeckSafetyApply(
                &exactConfiguration,
                exactConfiguration.panCenterTarget,
                (int)position.lowerTarget,
                (int)position.upperTarget,
                &exactPose
            )
            && exactPose.panTarget == exactConfiguration.panCenterTarget
            && exactPose.lowerTarget == position.lowerTarget
            && exactPose.upperTarget == position.upperTarget;
        if (!positionIsExact) {
            self.neckCommandSafetyStatus = [NSString stringWithFormat:
                @"Person tracking posture %@ is missing or outside the active safety configuration.",
                name];
            return ROBNeckCommandDispositionRejected;
        }
        [positions addObject:[position copyValue]];
    }

    self.personTrackingPostureSequenceActive = YES;
    self.personTrackingPostureAdvanceScheduled = NO;
    self.personTrackingPostureGeneration += 1;
    self.personTrackingPostureNames = [positionNames copy];
    self.personTrackingPosturePositions = [positions copy];
    self.personTrackingPostureIndex = 0;
    self.personTrackingPostureDeadline = now + 30.0;
    return [self advancePersonTrackingPostureSequence];
}
                        ...
                        source:(NSString *)source
{
    ...
    BOOL personTrackingPostureCommand =
        [source isEqualToString:kROBPersonTrackingPostureSource];
    BOOL safeStartupLiftCommand = safeStartupCommand
        && self.safeNeckStartupPhase == ROBSafeNeckStartupPhaseLifting
        && [source isEqualToString:kROBSafeNeckStartupLiftSource]
        && self.safeNeckStartupPhaseOne != nil
        && panTarget == self.safeNeckStartupPhaseOne.panTarget
        && lowerTiltTarget == self.safeNeckStartupPhaseOne.lowerTarget
    ...
    BOOL panWriteSucceeded = !panTargetChanged
        || [self sendMaestroTarget:(unsigned short)panResult.panTarget
                                              channel:0];
    ...
    if (panTargetChanged) {
        [self traceNeckWriteForSource:source firstChannel:0 targets:@[@(panResult.panTarget)]];
    }
    ...
    BOOL lowerWriteSucceeded = !lowerTargetChanged;
    BOOL upperWriteSucceeded = !upperTargetChanged;
    if (lowerTargetChanged) {
        ...
        if (coupledWriteSucceeded) {
            [self traceNeckWriteForSource:source firstChannel:1
                                 targets:@[@(boundedLower), @(leveledResult.upperTarget)]];
        }
        ...
        if (upperWriteSucceeded) {
            [self traceNeckWriteForSource:source firstChannel:2
                                 targets:@[@(leveledResult.upperTarget)]];
        }
    }
    ...
    if (mayMoveLower) {
        ...
        if (lowerTargetChanged) {
            if (boundedLower == ROBNeckSafetyTargetOff) {
                ...
                self.commandedLowerNeckTargetReadyAt = now
                    + lowerMotionDuration
        ...
        if (boundedLower == ROBNeckSafetyTargetOff) {
            ...
            if (panTarget == ROBNeckSafetyTargetOff
                ...
                self.commandedNeckPanTarget = ROBNeckSafetyTargetOff;
                [self traceNeckWriteForSource:source firstChannel:0 targets:@[@0]];
                self.neckPanCommandKnown = YES;
                self.commandedNeckPanTargetReadyAt = 0;
                self.commandedNeckPanDegrees = NAN;
                self.neckPanCommandLimited = NO;
    ...
    if (personTrackingPostureCommand) {
        [status addObject:@"PERSON TRACKING EXACT POSTURE"];
    }
...

{
    ...
    BOOL personTrackingUprightOwnsNeck = !operatorInitiated
        && [self.neckCommandSource
            isEqualToString:kROBPersonTrackingUprightSource]
        && self.personTrackingUprightTransitionActive;
    BOOL personTrackingPostureOwnsNeck = !operatorInitiated
        && [self.neckCommandSource
            isEqualToString:kROBPersonTrackingPostureSource]
        && self.personTrackingPostureSequenceActive;
    // A centering correction is already on the wire. Its mirrored sliders are
    ...
    if (!routineOwnsNeck
        ...
        && !personTrackingPostureOwnsNeck
        && !personCenteringOwnsNeck
        && torsoMayResumeNeck) {
